...
def _mulaw_to_pcm16k(mulaw_b64: str) -> str:
    ...
    Twilio sends mulaw 8-bit 8kHz; ElevenLabs expects PCM 16-bit 16kHz.
    """
    mulaw_bytes = base64.b64decode(mulaw_b64)
    # mulaw → PCM 16-bit (still 8kHz)
...
def _pcm16k_to_mulaw(pcm_b64: str) -> str:
    ...
    ElevenLabs sends PCM 16-bit; Twilio expects mulaw 8-bit 8kHz.
    """
...
async def twilio_stream(ws: WebSocket, meeting_id: str):
    ...
    Based on the reference implementation from elevenlabs-twilio-i-o.
    Twilio sends mulaw 8kHz base64; ElevenLabs handles the format natively
    when using their Twilio integration path.
    """
    settings = get_settings()
    await ws.accept()
    ...
    try:
        ...
        # Send minimal initialization — no overrides that could trigger policy violations
        init_msg = {
            ...
            try:
                while True:
                    ...
                    if msg.get("event") == "start":
                        ...
                        # Convert Twilio mulaw 8kHz → PCM 16-bit 16kHz for ElevenLabs
                        if el_ws:
                            mulaw_payload = msg["media"]["payload"]
                            try:
                                pcm_payload = _mulaw_to_pcm16k(mulaw_payload)
        ...
        async def elevenlabs_to_twilio():
            ...
            try:
                async for message in el_ws:
                    ...
                    if msg_type == "audio":
                        # Two possible audio formats from ElevenLabs
                        audio_data = None
                        ...
                        if audio_data and stream_sid:
                            try:
                                # Convert ElevenLabs PCM 16kHz → Twilio mulaw 8kHz
