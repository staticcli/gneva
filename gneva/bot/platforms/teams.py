"""Microsoft Teams web client driver."""
...
S = {
    ...
    # Guest join
    "name_input": "input[placeholder*='name' i], input[aria-label*='name' i], input#username",
    ...
    # In-meeting controls
    "mic_button": "button[aria-label*='Mute' i], button[id*='microphone' i]",
    "cam_button": "button[aria-label*='video' i], button[id*='video' i]",
    ...
    # Meeting state
    "lobby": "div:has-text('Someone in the meeting should let you in soon'), div:has-text('waiting in the lobby')",
    "meeting_ended": (
...
class TeamsDriver(BasePlatformDriver):
    """Microsoft Teams Web client meeting automation."""
    ...
    async def join(self, meeting_url: str) -> bool:
        ...
        # Toggle off mic and camera on preview
    ...
    async def detect_meeting_ended(self) -> bool:
        ...
        # Check participant count — if we're the only one, meeting is effectively over
        try:
