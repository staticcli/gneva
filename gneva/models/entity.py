...
from sqlalchemy import String, Integer, Float, ForeignKey, Text, Date, Boolean
...
class Entity(Base):
    ...
    metadata_json: Mapped[dict] = mapped_column(JSONB, server_default=sa_text("'{}'::jsonb"))
    embedding = mapped_column(JSONB, nullable=True)
    first_seen: Mapped[datetime] = mapped_column(server_default=sa_text("now()"))
    last_seen: Mapped[datetime] = mapped_column(server_default=sa_text("now()"))
...
class EntityRelationship(Base):
    ...
    confidence: Mapped[float] = mapped_column(Float, nullable=False, server_default=sa_text("1.0"))
    evidence: Mapped[str | None] = mapped_column(Text, nullable=True)
    meeting_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("meetings.id"), nullable=True)
    valid_from: Mapped[datetime] = mapped_column(server_default=sa_text("now()"))
...
class Contradiction(Base):
    ...
    status: Mapped[str] = mapped_column(String, nullable=False, server_default="open")
    detected_at: Mapped[datetime] = mapped_column(server_default=sa_text("now()"))
