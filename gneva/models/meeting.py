...
class TranscriptSegment(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(CompatUUID(), primary_key=True, default=new_uuid)
    transcript_id: Mapped[uuid.UUID] = mapped_column(CompatUUID(), ForeignKey("transcripts.id"), nullable=False)
    speaker_id: Mapped[uuid.UUID | None] = mapped_column(CompatUUID(), ForeignKey("users.id"), nullable=True)
...
class MeetingSummary(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(CompatUUID(), primary_key=True, default=new_uuid)
    meeting_id: Mapped[uuid.UUID] = mapped_column(CompatUUID(), ForeignKey("meetings.id"), unique=True, nullable=False)
    tldr: Mapped[str] = mapped_column(Text, nullable=False)
    key_decisions: Mapped[list | None] = mapped_column(JSON, default=list)
    action_items_json: Mapped[str] = mapped_column(Text, default="[]")
    topics_covered: Mapped[list | None] = mapped_column(JSON, default=list)
